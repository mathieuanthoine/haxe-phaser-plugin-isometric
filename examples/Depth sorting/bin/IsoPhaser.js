...
var Main = function() {
	this.game = new Phaser.Game(960,640,Phaser.AUTO,"game",{ preload : $bind(this,this.preload), create : $bind(this,this.create), update : $bind(this,this.update), render : $bind(this,this.render)});
};
Main.__name__ = true;
Main.main = function() {
	new Main();
...
Main.prototype = {
	...
	,create: function() {
		...
		while(xx > 0) {
			...
			while(yy > 0) {
				...
				cube.oldZ = cube.z;
				this.game.add.tween(cube).to({ isoZ : 10},100 * ((xx + yy) % 10),Phaser.Easing.Quadratic.InOut,true,0,Math.POSITIVE_INFINITY,true);
				yy -= 48;
			}
			xx -= 48;
		}
	...
	,onDown: function() {
		...
		if(this.sorted) this.game.iso.simpleSort(this.isoGroup); else this.isoGroup.sort("oldZ");
	}
	,update: function() {
	}
	,render: function() {
		this.game.debug.text("Click to toggle! Sorting enabled: " + Std.string(this.sorted),2,36,"#ffffff");
